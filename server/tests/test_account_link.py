...
class FakeLinkDb:
    ...
    def execute(self, sql, params=None):
        ...
        if sql.startswith("SELECT id FROM schedules WHERE user_id"):
            return FakeResult([])
        if sql.startswith("SELECT id, name, term, start_date, end_date, background, variant_type FROM schedules WHERE user_id"):
            return FakeResult([{"id": 1, "name": "默认课表", "term": "2026秋", "start_date": "2026-09-01", "end_date": "2027-01-20", "background": "", "variant_type": "original"}])
...
def test_unlink_migrates_identities_and_preserves_schedule():
    ...
    assert call("INSERT INTO schedules(user_id, name, term, start_date, end_date, background, variant_type)", db)
